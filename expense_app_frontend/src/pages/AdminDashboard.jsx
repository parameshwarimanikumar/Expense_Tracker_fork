...
const ExpenseDashboardTailwind = () => {
  ...
  useEffect(() => {
    const getExpenses = async () => {
      ...
      try {
        ...
        setError(error.response ? error.response.data.message : "Failed to fetch expenses.");
      }
    };
  ...
  const updateDashboardData = (data) => {
    ...
    data.forEach((exp) => {
      const type = exp.expense_type ? exp.expense_type.toLowerCase().trim() : "";
    ...
    setChartData([
      { name: "Regular Expense", value: regularExpense > 0 ? parseFloat(regularExpense.toFixed(2)) : 0 },
      { name: "Other Expense", value: otherExpense > 0 ? parseFloat(otherExpense.toFixed(2)) : 0 },
    ]);
  };
  ...
  const handleCheckboxChange = async (expenseId, field, checked) => {
    ...
    try {
      ...
      setError(error.response ? error.response.data.message : `Failed to update ${field}.`);
      setExpenses(originalExpenses);
      setFilteredExpenses(originalExpenses);
    }
  };
      ...
      <div className="flex-1 p-5 flex flex-col justify-between">
        ...
        <div className="flex justify-between gap-5 flex-1 overflow-hidden">
          <div className="grid grid-cols-3 gap-5 flex-1">
            {[
              { title: "Regular Expense", amount: `₹ ${cardData.regularExpense}` },
        ...
        <div className="bg-white p-4 rounded-xl h-[60vh] overflow-auto">
          <div className="font-bold text-[#1e2a52] text-lg mb-3">Recent Entries</div>
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {filteredExpenses.map((exp, index) => (
                <tr key={exp.id} className="text-[#1e2a52] border-t border-gray-200">
                  <td className="py-3 text-center">{index + 1}</td>
                  <td className="py-3 text-center">{dayjs(exp.date).format("DD/MM/YYYY")}</td>
                  <td className="py-3 text-center">{exp.user || "Unknown"}</td>
                  <td className="py-3 text-center">{exp.description || "N/A"}</td>
                  <td className="py-3 text-center">₹ {parseFloat(exp.amount).toFixed(2)}</td>
                  <td className="py-3 text-center">
                    <button
                      onClick={() => handleCheckboxChange(exp.id, "is_verified", !exp.is_verified)}
                  ...
                  <td className="py-3 text-center">
                    <button
                      onClick={() => handleCheckboxChange(exp.id, "is_refunded", !exp.is_refunded)}
                      className={`px-3 py-1 rounded-full text-xs font-semibold ${
                        exp.is_refunded
